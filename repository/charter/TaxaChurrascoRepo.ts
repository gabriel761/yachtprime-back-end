...
export class TaxaChurrascoRepository {
    async getTaxaChurrascoById(idTaxaChurrasco: number):Promise<TaxaChurrascoDatabase>{
       const result = await db.one("SELECT * FROM taxa_churrasco WHERE id=$1",[idTaxaChurrasco]).catch((error) => {
            throw new CustomError(`Repository lever Error: TaxaChurrascoRepository getTaxaChurrasco: ${error}`, 500)
        });
        if(!result){
            throw new CustomError(`Repository lever Error: TaxaChurrascoRepository getTaxaChurrasco: Taxa churrasco não encontrada`, 500)
        }
        return result
    }
    ...
    async deleteTaxaChurrasco(idTaxaChurrasco: number){
        db.query("DELETE FROM taxa_churrasco WHERE id=$1", [idTaxaChurrasco]).catch((error) => {
            throw new CustomError(`Repository lever Error: TaxaChurrascoRepository deleteTaxaChurrasco: ${error}`, 500)
        });
    }
}