...
export class CidadeRepository {
    async getCidades():Promise<Cidade[]>{
        const cidades = db.query(`SELECT * FROM cidade`).catch((error) => {
            throw new CustomError(`Repository lever Error: CidadeRepository getCidades:${error}`, 500)
        })

        if(!cidades){
            throw new CustomError(`Repository lever Error: CidadeRepository getCidades: Nenhuma cidade encontrada`,404)
        }
    ...
    async getIdCidadeByString(cidade: string):Promise<number>{
       const idCidade = await db.one(`SELECT id FROM cidade WHERE opcao = $1`, [cidade]).catch((error) => {
            throw new CustomError(`Repository lever Error: CidadeRepository getIdCidadeByString:${error}`, 500)
        })
